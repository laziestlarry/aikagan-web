...
import { isStorefrontCommerceEnabled } from "@/lib/commerce";
        ...
        <section className="mx-auto max-w-4xl">
          ...
          <div className="mt-10">
            {isScoped ? <Link href={`${prefix}/contact?product=${encodeURIComponent(product.slug)}`} className="inline-flex items-center gap-2 rounded-xl bg-amber-300 px-6 py-3.5 font-black text-black">{text.scope}<ArrowRight className="h-4 w-4" /></Link>
              : ready ? <CheckoutButton href={product.checkoutUrl} slug={product.slug} price={product.price} className="inline-flex items-center gap-2 rounded-xl bg-amber-300 px-6 py-3.5 font-black text-black">{text.open}<ArrowRight className="h-4 w-4" /></CheckoutButton>
