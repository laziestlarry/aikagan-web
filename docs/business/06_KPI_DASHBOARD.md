...
|--------|------------|-------------|---------------|---------------|-------|--------------|
| **Daily Revenue** | Sum of all Retired provider + Shopier + Shopify sales | $50 | $350 | $1,500 | < $10/day for 3 days | Increase ad spend, post more content |
...
| **Gross Margin** | (Revenue - Retired provider/Shopier fees - COGS) / Revenue | > 90% | > 92% | > 94% | < 88% | Check for unexpected costs |
...
| **Chargeback Rate** | Disputes / Orders | < 0.5% | > 1% | Retired provider may restrict. Check fraud. |
...
| **Checkout to Payment** | Completed / Started | 70%+ | < 50% | Check Retired provider errors, card acceptance |
...
| **Checkout API Response** | < 500ms | > 2s | Check Retired provider API latency |
| **Token Delivery Time** | < 3s from payment | > 10s | Check token-store fallback |
| **Webhook Success Rate** | 99%+ | < 95% | Check Retired provider Dashboard |
...
|----------|--------------|--------|
| Retired provider Checkout fails | < 5 min | Switch to backup payment method |
...
| Security breach | < 60 min | Revoke all keys, contact Retired provider |
...
|------|--------|-----------------|
| Revenue & Orders | Retired provider API (via webhook + dashboard) | Real-time |
...
### Daily (5 minutes)
```
□ Retired provider Dashboard: new sales, revenue
...
□ Cost analysis (Retired provider fees, AI API costs, hosting)
