...
export const metadata: Metadata = {
  title: 'İş Hedefini Net Bir Uygulama Planına Dönüştür',
...
const steps = [
  ...
  ['2', 'Sınırı ve hedefi belirleyin', 'Neye dokunulmaması gerektiğini, mevcut durumu ve tamam sayacağınız sonucu ekleyin.'],
...
export default function TurkishOutcomePage() {
  return <main className="min-h-screen bg-[#07090d] text-white">
    <section className="border-b border-white/10 px-6 py-24"><div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
      <div><p className="text-sm font-bold text-sky-300">ÜCRETSİZ İŞ HEDEFİ TASLAĞI</p><h1 className="mt-4 text-5xl font-black leading-[.98] sm:text-6xl">Bir iş sorununu <span className="text-amber-300">net bir uygulama görevine</span> dönüştürün.</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-neutral-300">Teknik terim bilmeniz gerekmez. Sorunu, hedefi ve sınırları anlatın; kimin karar vereceği ve hangi sonucun kabul edileceği belli bir taslak alın.</p><Link href="/tr/outcome/intake" className="mt-8 inline-flex items-center gap-2 rounded-xl bg-amber-300 px-6 py-4 font-black text-black">Ücretsiz taslağı hazırla <ArrowRight className="h-4 w-4"/></Link><div className="mt-7 flex flex-wrap gap-4 text-sm text-neutral-400">{['Satın alma yok','Taslak düzenlenebilir','Göndermeden önce son kontrol','İnsan onayı korunur'].map(x=><span key={x} className="inline-flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-emerald-300"/>{x}</span>)}</div></div>
      <aside className="rounded-3xl border border-sky-300/20 bg-white/[.03] p-7"><ShieldCheck className="h-7 w-7 text-emerald-300"/><h2 className="mt-5 text-2xl font-black">Yapay zekâ hazırlar. Sorumlu kişi karar verir.</h2><p className="mt-4 leading-7 text-neutral-400">Bu ücretsiz araç fikirleri düzenler ve bir sonraki adımı önerir. Ödeme yapmaz, hesabınıza girmez, içerik yayımlamaz ve sizin adınıza dış sistemlerde işlem başlatmaz.</p></aside>
    </div></section>
    <section className="mx-auto max-w-7xl px-6 py-24"><p className="text-sm font-bold text-amber-300">BAŞTAN SONA AYNI DİLDE</p><h2 className="mt-3 text-4xl font-black">Dört anlaşılır adım.</h2><div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{steps.map(([n,t,b])=><article key={n} className="rounded-2xl border border-white/10 bg-white/[.03] p-6"><span className="text-sm font-black text-sky-300">{n}</span><h3 className="mt-4 text-xl font-black">{t}</h3><p className="mt-3 text-sm leading-6 text-neutral-400">{b}</p></article>)}</div></section>
