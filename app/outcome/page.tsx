...
export default function OutcomeHome() {
  return <main className="min-h-screen bg-[#07090d] text-white">
    ...
    <section className="border-y border-white/10 bg-[#0b1019] px-6 py-24"><div className="mx-auto max-w-7xl"><p className="text-xs font-black uppercase tracking-[.24em] text-amber-300">Ready-made implementation assets</p><h2 className="mt-4 max-w-3xl text-4xl font-black">Choose a pack only when the mission is clear.</h2><p className="mt-5 max-w-3xl leading-7 text-neutral-400">Each button opens the named Gumroad product through AIKAGAN’s recorded checkout handoff. Gumroad processes payment and issues the purchase record used for delivery.</p><div className="mt-10 grid gap-5 lg:grid-cols-3">{[
      ['BizOp Navigator — Starter','$29','masterclass-starter','One reviewed decision brief and a focused seven-day action plan.'],
      ['Revenue System — Pro','$79','masterclass-pro','Offer, traffic, checkout and a 30-day revenue operating cadence.'],
      ['Operating System — Commander','$149','masterclass-commander','Repeatable operations, human approval gates, partnerships and KPI controls.'],
      ...
      <div className="mt-5 rounded-3xl border border-white/10 bg-white/[.03] p-7">
        <p className="text-xs font-black uppercase tracking-[.2em] text-emerald-300">Managed operations</p>
        <h3 className="mt-3 text-2xl font-black">Keep it running — $199/mo request-scope</h3>
        <p className="mt-4 max-w-3xl text-sm leading-7 text-neutral-400">Continuous monitoring and monthly evidence review after E5 acceptance. This is not an automated checkout SKU. Scope, access and acceptance are confirmed before any recurring fee is invoiced.</p>
        <Link href="/contact?product=managed-ops" className="mt-6 inline-flex items-center gap-2 text-sm font-black text-amber-300">Request managed scope <ArrowRight className="h-4 w-4"/></Link>
      </div>
      <p className="mt-7 text-xs text-neutral-500">Payment does not authorize unrelated account, publishing, legal or destructive actions. Those remain separate human approval gates. Public proof publishes only at E4 Live or E5 Accepted.</p></div></section>
