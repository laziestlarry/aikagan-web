...
const supported: SocialNetwork[] = ['linkedin', 'x', 'facebook'];
...
export async function POST(req: NextRequest) {
  ...
  const { networks, text, url } = await req.json();
  ...
  const results = await Promise.allSettled(requested.map((network) => publishToNetwork(network, { text, url })));
