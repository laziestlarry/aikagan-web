...
const TURKISH_NAV = [
  ...
  { label: 'Ticaret Stüdyosu', href: '/urunler' },
...
export default function Navbar({ locale = 'en' }: { locale?: Locale }) {
  ...
  const languageHref = isTurkish
    ? `https://aikagan.com${TR_TO_EN[currentPath] || '/'}?lang=en`
    : `https://aikagan.com${EN_TO_TR[currentPath] || '/tr'}?lang=tr`;
  ...
  return <nav className="sticky top-0 z-50 border-b border-kagan-border/60 bg-kagan-black/90 backdrop-blur-lg">
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="flex h-16 items-center justify-between">
      ...
      <div className="hidden md:flex items-center gap-3">{!isAppRoute&&<Link href={languageHref} aria-label={isTurkish?'Switch to English':'Türkçeye geç'} className="inline-flex items-center gap-1.5 rounded-xl border border-kagan-gold/35 bg-kagan-gold/5 px-3.5 py-2 text-xs font-bold text-kagan-gold transition hover:border-kagan-gold/60 hover:bg-kagan-gold/10 hover:text-kagan-white"><Globe2 className="h-3.5 w-3.5" />{isTurkish?'EN · Global':'TR · Türkiye'}</Link>}{isAppRoute?<Link href={`${SITE.url}/tools`} className="inline-flex items-center gap-2 rounded-lg border border-kagan-gold/40 px-4 py-2 text-sm font-semibold text-kagan-gold hover:bg-kagan-gold/10">Try Free <ArrowUpRight className="h-4 w-4"/></Link>:<Link href={isTurkish?'/tr/tools/revenue-leak-scan':`${SITE.url}/tools/revenue-leak-scan`} className="inline-flex items-center gap-2 rounded-lg bg-kagan-gold px-4 py-2 text-sm font-semibold text-black hover:bg-kagan-gold-light">{isTurkish?'Ücretsiz Başla':'Start Free Scan'} <ArrowUpRight className="h-4 w-4"/></Link>}</div>
    ...
    {open&&<div className="md:hidden border-t border-kagan-border/60 bg-kagan-black/95"><div className="px-4 py-4 space-y-1">{links.map((link)=><Link key={link.label} href={normalizeHref(link.href)} onClick={()=>setOpen(false)} className="block px-3 py-2 rounded-lg text-sm font-medium text-kagan-light hover:text-kagan-white hover:bg-kagan-card">{link.label}</Link>)}{!isAppRoute&&<Link href={languageHref} onClick={()=>setOpen(false)} className="flex items-center gap-2 rounded-xl border border-kagan-gold/25 bg-kagan-gold/5 px-3 py-2.5 text-sm font-bold text-kagan-gold"><Globe2 className="h-4 w-4" />{isTurkish?'English · Global':'Türkçe · Türkiye'}</Link>}<Link href={isAppRoute?`${SITE.url}/tools`:isTurkish?'/tr/tools/revenue-leak-scan':`${SITE.url}/tools/revenue-leak-scan`} onClick={()=>setOpen(false)} className="block mt-2 text-center rounded-lg bg-kagan-gold px-4 py-3 text-sm font-semibold text-black">{isAppRoute?'Try Free':isTurkish?'Ücretsiz Başla':'Start Free Scan'}</Link></div></div>}
  </nav>;
}
