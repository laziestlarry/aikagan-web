...
export default function Navbar({ locale = 'en' }: { locale?: Locale }) {
  ...
  const isTurkish = locale === 'tr' && (!isAppRoute || pathname.startsWith('/checkout'));
  ...
  return <nav className="sticky top-0 z-50 border-b border-kagan-border/60 bg-kagan-black/90 backdrop-blur-lg">
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="flex h-16 items-center justify-between">
      ...
      <div className="hidden xl:flex items-center gap-2">{!isAppRoute&&<Link href={languageHref} aria-label={isTurkish?'İngilizceye geç':'Türkçeye geç'} className="inline-flex items-center gap-1.5 rounded-xl border border-kagan-gold/35 bg-kagan-gold/5 px-3 py-2 text-xs font-bold text-kagan-gold transition hover:border-kagan-gold/60 hover:bg-kagan-gold/10 hover:text-kagan-white"><Globe2 className="h-3.5 w-3.5" />{isTurkish?'EN':'TR'}</Link>}{isAppRoute?<Link href={`${SITE.url}/tools`} className="inline-flex items-center gap-2 rounded-lg border border-kagan-gold/40 px-4 py-2 text-sm font-semibold text-kagan-gold hover:bg-kagan-gold/10">Try Free <ArrowUpRight className="h-4 w-4"/></Link>:<Link href={isTurkish?`${SITE.url}/tr/tools/revenue-leak-scan`:`${SITE.url}/tools/revenue-leak-scan`} className="inline-flex items-center gap-2 rounded-lg bg-kagan-gold px-3 py-2 text-sm font-semibold text-black hover:bg-kagan-gold-light">{isTurkish?'Ücretsiz Test':'Start Free Scan'} <ArrowUpRight className="h-4 w-4"/></Link>}</div>
