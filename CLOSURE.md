...
| 3 | Intake form "contact us at the email below" with no email below | `app/contact/page.tsx` rewritten: (a) when `NEXT_PUBLIC_FORMSPREE_ID` is unset, the form POSTs to `/api/lead` AND opens a pre-filled `mailto:kagan@aikagan.com` so the submission always reaches us; (b) added a permanent "Prefer email? Write us directly at kagan@aikagan.com" line under the form so the email is always visible. |
